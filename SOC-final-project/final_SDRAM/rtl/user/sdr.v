...
module sdr (
    ...
    parameter COL_BITS          = A_WIDTH;   // # of COL module sdr
...
*/
    ...
    */
    ...
    Bank0(
        ...
        .we(bwen[0]), 
        .re(bren[0]), 
        .waddr(Col_brst[A_WIDTH-1:0]), 
        .raddr(Col_brst[A_WIDTH-1:0]), 
        .d(bdi[0]), 
        .q(bdq[0])
    );
    blkRam#(.SIZE(mem_sizes), .BIT_WIDTH(DQ_BITS))
    ...
    // Detect Read or Write command
            if (Read_enable || Command[0] == `WRITE) begin//Command[2] == `READ
                if(Read_enable)begin//Command[2] == `READ
					...
					case (Ba)//Bank_addr[2]
                        2'b00 : Col_brst <= {B0_row_addr,Addr[9:2]};
                        2'b01 : Col_brst <= {B1_row_addr,Addr[9:2]};
                        2'b10 : Col_brst <= {B2_row_addr,Addr[9:2]};
                        2'b11 : Col_brst <= {B3_row_addr,Addr[9:2]};
                    endcase
                    Burst_counter <= 0;
					...
					case (Bank_addr[0])//Bank_addr[2]
                        2'b00 : Col_brst <= {B0_row_addr,Col_addr[0][9:2]};
                        2'b01 : Col_brst <= {B1_row_addr,Col_addr[0][9:2]};
                        2'b10 : Col_brst <= {B2_row_addr,Col_addr[0][9:2]};
                        2'b11 : Col_brst <= {B3_row_addr,Col_addr[0][9:2]};
                    endcase
                    Burst_counter <= 0;
                end
                //Data_in_enable = 1'b0;
